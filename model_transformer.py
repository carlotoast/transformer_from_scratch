...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0)/d_model)) # Operation in this line (d_model / 2)

        # Apply the sine to even positions
        pe[:,0::2] = torch.sin(position * div_term)

        # Apply cosine to odd indicies
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, features: int, self_attention_block: MultiHeadAttentionBlock, feed_forward_block: FeedforwardBlock, dropout: float) ->None:
        super().__init__()
        self.self_attentiona_block = self_attention_block
        self.feed_forward_block = feed_forward_block
        self.residual_connections = nn.ModuleList([ResidualConnection(features, dropout) for _ in range (2)])
        
    def forward(self, x, src_mask):
        x = self.residual_connections[0](x, lambda x:self.self_attentiona_block(x, x, x, src_mask))
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff = 2048) -> Transformer:
    # d_model =512; N(layer) = 6; h(hyperparameter) = 8; doprout = 0.1; d_ff = 2048. All of these parameters came from the paper

    # Creat the embedding layers
    src_embed = Inputembeddings(d_model, src_vocab_size)
    tgt_embed = Inputembeddings(d_model, tgt_vocab_size)

    # Create the positon encoding layers
